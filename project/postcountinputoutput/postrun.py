...
import os
import csv
from post_count import PostCount
...
def run_mrjob(input_file, output_dir):
    ...
    try:
        ...
        mr_job = PostCount(args=['-r', 'hadoop', input_file, '--output-dir', output_dir])
        ...
        with mr_job.make_runner() as runner:
            ...
            final_output_path = f'{output_dir}/output_platform_post_counts.csv'
            with open(temp_output_filename, 'r') as infile:
                with open('final_output.csv', 'w', newline='') as csvfile:
                    ...
                    for line in infile:
                        platform, count = line.strip().split('\t')
                        writer.writerow([platform, count])
